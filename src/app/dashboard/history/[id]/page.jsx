...
import { getHistoryScanById } from "@/lib/historyService";
import ScanResultCard from "@/components/dashboard/ScanResultCard";

// Fallback only used when the scan type has no explicit recommendation of
// its own (URL scans carry one on urlScan.recommendation; email/message
// scans may not). Mirrors RiskScoreService.getRecommendation's thresholds
// on the backend, so it's an approximation, not a guess.
...
// The /history/:id findings come back camelCase (findingType, findingValue)
// but ScanResultCard expects snake_case (finding_type, finding_value) —
// normalize here rather than changing the shared component.
...
export default function HistoryScanDetailPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    (async () => {
      ...
      setErrorMessage("");
      try {
        const res = await getHistoryScanById(id);
        setData(res.data);
      } catch (err) {
